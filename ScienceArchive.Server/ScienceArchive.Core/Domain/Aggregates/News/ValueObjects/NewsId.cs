...
public sealed class NewsId : EntityId<Guid>
{
    ...
    public static NewsId CreateNew()
    {
    ...
    /// </exception>
    public static NewsId CreateFromString(string value)
    {
        if (!Guid.TryParse(value, out var idValue))
        {
        	throw new InvalidEntityIdValueException(value, nameof(NewsId));
        }
    ...
    /// </exception>
    public static NewsId CreateFromGuid(Guid value)
    {
    ...
    {
        return compareId is NewsId compareNewsId && Value.Equals(compareNewsId.Value);
    }
}